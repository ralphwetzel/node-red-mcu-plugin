...
class manifest_builder {
    ...
    create_manifests_for_module(module, destination, node_type) {
        ...
        //     check if there's a node-red section in package.json that declares a file for this node type!
        if (node_type) {
            if (pckge["node-red"]?.nodes?.[node_type]) {
                _file = pckge["node-red"].nodes[node_type];
            }
        }
        ...
        _file = _file ?? pckge.exports?.import ?? pckge.exports?.require ?? pckge.main ?? "./index.js";
    ...
    from_template(module, destination) {
        ...
        let mt = clone(mnfst_template);
        delete mt["//"].template;
        let bldr = new manifest_builder(self.nodes_library, self.mcu_modules_path);
        bldr.initialize(mt);
